...
const getParticularBooks = async function(req, res) {
    let getObj = req.body
    let book = await BookModel.find(getObj)
    res.send({ msg: book })
}

// 5.
const getXINRBooks = async function(req, res) {
    const getINR = await BookModel.find({ 'price.indianPrice': { $in: ["300INR", "400INR", "500INR"] } });
    res.send({ data: getINR })
}

// 6.
const getRandomBooks = async function(req, res) {
    const randomBook = await BookModel.find({ stockAvailable: true, totalPages: { $gt: 300 } });
    res.send({ data: randomBook });
}
